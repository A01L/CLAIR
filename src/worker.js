...
import { indexAppeal } from "./search/elastic.js";
import { sendToDLQ } from "./queue/rabbit.js";
import { analyzeAndCreateAppealJob } from "./controllers/appeals.js";
...
async function handleMessage(ch, msg, queueName) {
  ...
  try {
    ...
    if (payload?.type === "APPEAL_ANALYZE") {
      const newAppeal = await analyzeAndCreateAppealJob(payload.data);
      if (newAppeal && !newAppeal.skipped) { await indexAppeal(newAppeal); }
    } else {
      console.warn("⚠ Unknown job type:", payload?.type);
    }

    await invalidateCachePattern("appeals:*");
    await invalidateCachePattern("reports:*");
    ch.ack(msg);
    ...
    await sendToDLQ(channelId, msg.content, {
      ...(msg.properties?.headers || {}),

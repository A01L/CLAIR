...
export function buildDLQName(channelId) {
  const cid = toSafeNumber(channelId, 0);
  return `appeals_channel_${cid}_dlq`;
}
...
export async function ensureChannelQueues(channelId) {
  ...
  // Safely define the main queue without changing existing arguments to prevent 406 Precondition Failed on existing queues.
  await ch.assertQueue(mainQueue, { durable: true });
  await ch.assertQueue(buildDLQName(channelId), { durable: true });
...
export async function sendToDLQ(channelId, content, headers = {}) {
  const ch = await initRabbit();
  const dlq = buildDLQName(channelId);
  await ch.assertQueue(dlq, { durable: true });
  ch.sendToQueue(dlq, Buffer.from(content), {
    persistent: true,
    headers
  });
}
